...
function wrapWith(element, config) {
  ...
  const nodes = document.createTreeWalker(
    ...
    // optional: Accept node always.  Same as 'no function' here.
    {
  ...
  while (node = nodes.nextNode()) {
    ...
    while (m = text.match(re)) {
      ...
      matchCb(m);
      const front = text.slice(0, m.index); // might be empty ''
      const mid = m[0];
      const end = text.slice(m.index + mid.length); // might be empty ''
      text = end;
      // todo this line is dangerous
      p.insertBefore(document.createTextNode(front), node);
      const word = p.insertBefore(document.createElement(wrapTag), node);
